            ...
            <div class="flex">
               ...
               @auth
                @if(Auth::user()->hasRole('superAdmin'))
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('admin.fundraising.index')" :active="request()->routeIs('admin.fundraising.index')">
                            {{ __('Fundraising') }}
                        </x-nav-link>
                    </div>
                ...
                @else
                    ...
                    @php
                        // Query untuk mendapatkan nilai is_active
                        $isActive = \DB::table('fundraisers')
                            ->where('users_id', Auth::id())
                            ->value('is_active'); // Mengambil satu kolom is_active
                    @endphp

                    @if(!is_null($isActive) && $isActive == 1)
                        <!-- Navigation for Active Fundraisers -->
                        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                            <x-nav-link :href="route('fundraisers.fundraisings.index')" :active="request()->routeIs('fundraisers.fundraisings.index')">
                                {{ __('Fundraisings') }}
                            </x-nav-link>
                        </div>
            ...
            {{-- avatar --}}
                    ...
                    <x-slot name="trigger">
                        ...
                        <div class="inline-flex items-center space-x-2">
                            <img src="{{ asset('avatar/' . ($avatarName ?? 'default-avatar.png')) }}" alt="Avatar" class="rounded-full w-8 h-8">
